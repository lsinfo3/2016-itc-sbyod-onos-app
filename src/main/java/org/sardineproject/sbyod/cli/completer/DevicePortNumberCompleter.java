 ...
 */
public class DevicePortNumberCompleter implements Completer {
    @Override
    public int complete(String buffer, int cursor, List<String> candidates) {
        ...
        while (it.hasNext()) {
            Iterator<Port> portIt = service.getPorts(it.next().id()).iterator();
            while(portIt.hasNext())
                strings.add(portIt.next().number().toString());
        }
