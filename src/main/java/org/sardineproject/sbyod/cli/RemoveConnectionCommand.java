...
public class RemoveConnectionCommand extends AbstractShellCommand {
    ...
    protected void execute() {
        try {
            ...
            if (connection == null){
                System.out.println(String.format("No connection between user = %s and service = %s found.",
                        user, service));
            }else {
                connectionStore.removeConnection(connection);
                System.out.println(String.format("Connection %s removed", connection));
            }

        } catch (Exception e){
            System.out.println(String.format("Could not remove connection between userId = %s and serviceId = %s",
                    userId, serviceId));
        }
    }
}